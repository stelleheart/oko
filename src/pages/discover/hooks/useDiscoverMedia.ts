...
export function useDiscoverMedia({
	...
	const fetchMedia = useCallback(async () => {
		...
		// ponytail: removed unused getTraktProviderFn dep (it was rebuilt every render
		// and not actually called inside fetchMedia).
	}, [
		contentType,
		mediaType,
		id,
	...
	// Reset media only when the content type actually changes (not on every page bump).
	// ponytail: `page === 1` triggered a wipe + re-fetch on every render where any
	// dep changed; the fetch effect below also re-fired for the same reason.
	useEffect(() => {
		if (contentType !== currentContentType) {
			setMedia([]);
			setCurrentContentType(contentType);
