...
export interface FetchRecommendationsParams {
  ...
  // Wrapped signature: (endpoint, params?) — the same shape the
  // orchestrator hook passes via its useCallback.
  fetchTMDBMedia: (
    endpoint: string,
...
export async function fetchRecommendationsWithFedSimilar({
  mediaId,
  mediaType,
  isCarouselView,
  page,
