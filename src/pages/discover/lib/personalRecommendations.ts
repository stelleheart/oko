...
export async function fetchPersonalRecommendations(
	...
	if (fedSimilarResults.status === "fulfilled") {
		...
		// Fetch full details for fed-similar items - dedup ids we already have from TMDB below.
		// ponytail: previous code hit getMediaDetails for every fed-similar id; with N sources
		// that's N * ~5 ids = up to 40 round-trips. Cache results so the TMDB path can reuse them.
		const tmdbRelatedById = new Map<
			number,
		...
		if (tmdbResults.status === "fulfilled") {
			for (const result of tmdbResults.value) {
				for (const item of result) {
					if (!tmdbRelatedById.has(item.id)) tmdbRelatedById.set(item.id, item);
				}
			}
		}
		...
		// Inject TMDB-related items that weren't already covered by fed-similar.
		// ponytail: previous code appended TMDB results directly even if the same id was
		// already in `seenIds` from fed-similar (the dedup happened inside the inner
		// loop) - but fed-similar items hadn't populated `seenIds` yet at that point.
		// Now we add them only if not seen.
		for (const item of tmdbRelatedById.values()) {
		...
		// No fed-similar results - process TMDB results the same way.
		if (tmdbResults.status === "fulfilled") {
			for (const result of tmdbResults.value) {
				for (const item of result) {
					const idStr = String(item.id);
