...
export function Modal(props: { id: string; children?: ReactNode }) {
	...
	// ponytail: subscribe to the slice, not the whole store. Use the id->index
	// map instead of running indexOf on every render.
	const modalIndex = useOverlayStack((s) => s.modalIndex[props.id]);
	const zIndex = modalIndex !== undefined ? 1000 + modalIndex : 999;
