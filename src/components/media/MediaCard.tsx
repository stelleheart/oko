...
import { useAllBookmarkGroups, useBookmarkStore } from "@/stores/bookmarks";
		...
		<div ref={targetRef as React.RefObject<HTMLDivElement>}>
			...
			>
				...
				>
					...
					>
						{/* ponytail: use <img loading=lazy> instead of `bg-cover background-image`.
                The browser handles off-screen decoding + paint on the GPU; the
                previous `style={{ backgroundImage }}` blew up the compositor
                for every card on the discover page (~160 cards). */}
						{isIntersecting ? (
							<img
								src={media.poster || "/placeholder.png"}
								alt={media.title || ""}
								loading="lazy"
								decoding="async"
...
export function MediaCard(props: MediaCardProps) {
	...
	// ponytail: shared memo via useAllBookmarkGroups — recomputed once per
	// bookmarks change instead of once per mounted MediaCard.
	const allGroups = useAllBookmarkGroups();
