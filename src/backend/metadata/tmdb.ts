...
// Mapping tables between MWMediaType / TMDBContentTypes / MediaItem["type"].
// Unknown inputs throw to preserve the original throw-based contract.
const MW_TO_TMDB: Partial<Record<MWMediaType, TMDBContentTypes>> = {
	[MWMediaType.MOVIE]: TMDBContentTypes.MOVIE,
	[MWMediaType.SERIES]: TMDBContentTypes.TV,
};
const MEDIA_ITEM_TO_MW: Partial<Record<MediaItem["type"], MWMediaType>> = {
	movie: MWMediaType.MOVIE,
	show: MWMediaType.SERIES,
};
const TMDB_TO_MW: Partial<Record<TMDBContentTypes, MWMediaType>> = {
	[TMDBContentTypes.MOVIE]: MWMediaType.MOVIE,
	[TMDBContentTypes.TV]: MWMediaType.SERIES,
};
const TMDB_TO_MEDIA_ITEM: Partial<Record<TMDBContentTypes, MediaItem["type"]>> =
	{
		[TMDBContentTypes.MOVIE]: "movie",
		[TMDBContentTypes.TV]: "show",
	};
...
export const mediaTypeToTMDB = (type: MWMediaType): TMDBContentTypes =>
	pick(MW_TO_TMDB, type);

export const mediaItemTypeToMediaType = (
	type: MediaItem["type"],
): MWMediaType => pick(MEDIA_ITEM_TO_MW, type);

export const TMDBMediaToMediaType = (type: TMDBContentTypes): MWMediaType =>
	pick(TMDB_TO_MW, type);

export const TMDBMediaToMediaItemType = (
	type: TMDBContentTypes,
): MediaItem["type"] => pick(TMDB_TO_MEDIA_ITEM, type);
