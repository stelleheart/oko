...
export function useRouterAnchorUpdate(id: string) {
	...
	const lastRect = useRef<string | null>(null);
	...
	const update = useCallback(() => {
		...
		if (anchor) {
			...
			// ponytail: skip setAnchorPoint when the rect didn't change. Resize
			// listeners fire on every pixel of drag; this collapses hundreds of
			// identical global store writes to one.
	...
	useEffect(() => {
		// ponytail: rAF-throttle the resize listener. Window resize can fire
		// hundreds of times during a drag; rAF collapses each frame to a single
		// getBoundingClientRect + setAnchorPoint.
		let frame = 0;
		function onResize() {
			if (frame) return;
