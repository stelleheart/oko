	...
	const syncData = useCallback(
		async (
			...
			if (settings.customTheme) {
				const resolvedTheme = resolveCustomTheme(settings.customTheme);
				if (resolvedTheme) setCustomTheme(resolvedTheme);

				if (settings.customTheme.savedCustomThemes) {
					settings.customTheme.savedCustomThemes.forEach((t) => {
						saveCustomTheme(t);
					});
				}
				if (settings.customTheme.hiddenDefaultThemes) {
					settings.customTheme.hiddenDefaultThemes.forEach((t) => {
						hideDefaultTheme(t);
					});
				}
			}
