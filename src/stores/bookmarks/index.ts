...
// ponytail: derived selector. Computing the global group set per MediaCard
// (the previous approach) was O(cards * bookmarks). One shared memo across
// all consumers, recomputed only when the bookmarks record changes.
export function useAllBookmarkGroups(): string[] {
	const bookmarks = useBookmarkStore((s) => s.bookmarks);
	return useMemo(() => {
		const set = new Set<string>();
		for (const bookmark of Object.values(bookmarks)) {
			if (bookmark.group) {
				for (const group of bookmark.group) set.add(group);
			}
		}
